...
// type School
// func New() *School
// func (students *School) Add(string, int)
// func (students *School) Grade(int) []string
// func (students *School) Enrollment() []Grade
...
func TestAddDifferentGrades(t *testing.T) {
	...
	got := list(s.Enrollment())
	if got != exp {
		t.Errorf(`Add different gradeList, got
%sexpected:
%students`, got, exp)
	}
}
...
func TestNonExistantGrade(t *testing.T) {
	...
	got := s.Grade(1)
	if len(got) != 0 {
		t.Errorf(`Get non-existent no, got
%q
expected
[]`, got)
	}
...
func BenchmarkAddStudents(b *testing.B) {
	const pool = 1e6 // pool of studentList
	...
	for i := 0; i < b.N; i++ {
		...
		// a number of studentList, drawn from a pool of studentList
		s := New()
